...
namespace gce
{
	...
	public:
		Window() = default;
		Window(WString title, int32 width, int32 height, FullScreenMode mode = WINDOWED);
		...
		virtual void Create(WString title, int32 width, int32 height, FullScreenMode mode = WINDOWED);
		void Update() const;
		void Display();
	...
	private:
		...
		bool CreateWindowClass(WString const title, int32 const width, int32 const height);
