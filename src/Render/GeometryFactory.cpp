...
#include "Gameplay/RessourceManager.h"
...
Vertex v[24]{
	...
	Geometry* GeometryFactory::LoadGeometry(StringView const path)
	{
		...
		for (int i = 0; i < obj.meshs.Size(); i++)
		{
			for (obj::Vertex& v : obj.vertices)
			{
				...
				ve.uv = { v.textureCoordinate.x, v.textureCoordinate.y }; // a verifier si 1- necessaire ou si plus besoin
	...
	Geometry* GeometryFactory::LoadJsonGeometry(json const& object)
	{
		...
		if (object.contains("stock") && object["stock"].is_boolean() && object["stock"].get<bool>() == true)
		{
			customGeo = RessourceManager::GetGeoWithName(object["name"]);

			if (customGeo == nullptr)
			{
				customGeo = new Geometry(vertices.Data(), vertices.Size(), indices.Data(), indices.Size());
				RessourceManager::AddGeometry(customGeo, object["name"]);
			}
		}
		else
		{
