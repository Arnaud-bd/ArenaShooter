...
void BoxCollider::Init()
{
	...
	m_worldBox.min = geo->min * localScale * transform.GetWorldScale() + GetOwner().transform.GetLocalPosition();
	m_worldBox.max = geo->max * localScale * transform.GetWorldScale() + GetOwner().transform.GetLocalPosition();
	localOBBPoints = Physics::GetListFromMinMax(m_worldBox.min, m_worldBox.max);
