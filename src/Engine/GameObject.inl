...
inline EntityType GameObject::GetType() const { return mType; }
inline Scene* GameObject::GetScene() { return m_pScene; }
    
inline void GameObject::SetName(cstr name) { m_name = name; }
inline void GameObject::SetType(EntityType type) { mType = type; }
...
ScriptClass* GameObject::GetScript()
{
    ...
    {
        Script* base = Script::s_list[scriptID.second];

        // si c'est du type demandé ou dérivé → dynamic_cast renvoie non-null
        if (auto casted = dynamic_cast<ScriptClass*>(base))
            return casted;
    }
    return nullptr;
}
