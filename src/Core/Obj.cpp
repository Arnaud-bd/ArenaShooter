...
namespace gce {
	...
	void Obj::LoadJsonObj(json const& object)
	{
		...
		for (int i = 0; i < loopCount; i++)
		{
			...
			// NORMAL : par loop (comme UV)
			Vector3f32 normal;
			normal.x = normals[i * 3 + 0].get<float>();
			normal.y = normals[i * 3 + 1].get<float>();
			normal.z = normals[i * 3 + 2].get<float>();

			vertices.PushBack(obj::Vertex(pos, normal, texCoord));
		}

		// Rebuild index buffer (1:1 with vertices)
		Mesh m;
